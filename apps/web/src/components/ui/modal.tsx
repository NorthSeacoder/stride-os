  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Popup className="fixed left-1/2 top-1/2 z-50 max-h-[min(760px,calc(100vh-2rem))] w-[min(680px,calc(100vw-2rem))] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-[22px] border border-(--border-glow) bg-(--bg-surface-2) shadow-[var(--shadow-shell)]">
          <div className="flex items-start justify-between gap-4 border-b app-shell-divider px-6 py-5">
            <div>
              <p className="text-[11px] uppercase tracking-[0.22em] text-(--text-muted)">编辑面板</p>
