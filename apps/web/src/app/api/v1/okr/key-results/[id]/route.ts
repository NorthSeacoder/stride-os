...
import { getOkrActivityContext, parseKeyResultInput, requireId, requireOkrApiUser } from '../../_lib';
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const keyResult = await updateKeyResult(id, input, { activityContext: getOkrActivityContext(user) });
    if (!keyResult) return NextResponse.json({ error: 'Not found' }, { status: 404 });
    return NextResponse.json(keyResult);
  } catch (error) {
